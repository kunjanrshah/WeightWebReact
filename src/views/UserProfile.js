...
function User() {
  ...
  let [weightMachineType, setweightMachineType] = useState('weight1');

  function handleWeightMachineChange(machineType){
    setweightMachineType(machineType);
  }

  function checkIsRouteParamsExist(){
      let pendingId=params.id;
  ...
  //Entry Type
  let [entryType, seteEntryType] = useState('auto');
  ...
  //setting
  const [settingFlag1, setsettingFlag1] = useState(true);
  const [settingFlag2, setsettingFlag2] = useState(true);
  ...
  useEffect(() => {
    if(entryType=='auto')
    {
      ...
      if(weightType=='tare')
      {
        if(weightMachineType=='weight1')
        {
          setdefaultTareWeight(+weight1);
        }
        else
        {
          setdefaultTareWeight(+weight2);
        }
  ...
  },[weightType,entryType,weight1,weight2]);
 ...
 {
    let settingbaseUrl="setting";
    const settingData = await ApiService(`${settingbaseUrl}?page=1&take=10`, 'GET', null);

    let flag1Data=settingData.filter((item)=>item.name=='flag1').pop().value;
    let flag2Data=settingData.filter((item)=>item.name=='flag2').pop().value;
 
    if(flag1Data=='ACTIVE')
    {
      setsettingFlag1(true);
    }
    else
    {
      setsettingFlag1(false);
    }

    if(flag2Data=='TARE-GROSS')
    {
      setsettingFlag2(true);
    }
    else
    {
      setsettingFlag2(false);
    }
      ...
      <Container fluid>
        <Row>
          <Col md="12">
            <Card>
              <Card.Header>
                <Card.Title as="h4">Weight Detail 
                <button className="btn btn-primary float-right" onClick={handleResetMachine}>RESET Scale</button>
                </Card.Title>
              </Card.Header>
              <Card.Body>
                <Form>
                  ...
                  <Row className="align-items-center">
                  <Col className="pr-1" md="3" style={{display: settingFlag2? 'block': 'none'}}>
                        <Form.Check
                          inline
                          label="Tare Weight"
                          name="weight"
                        ...
                        <Form.Check
                          ...
                          type={'radio'}
                          id={`inline-weight-2`}
                          checked={weightType=='gross'}
                          onChange={()=>{handleWeightTypeChange('gross')}}
                        />
                    </Col>
                    <Col className="pr-1" md="3" style={{display: settingFlag1? 'block': 'none'}}>
                        <Form.Check
                          inline
                          label="Auto"
                          name="weighing_type"
                    ...
                    <Col className="pl-3" md="6" >
                        <div className="form-check" style={{float: 'left'}}>
                          <input className="form-check-input" type="radio" name="weight_machine" id="weight_machine1" 
                          checked={weightMachineType=='weight1'}  onChange={()=>{handleWeightMachineChange('weight1')}}
                          />
                          <label className="form-check-label" htmlFor="weight_machine1" style={{'fontSize': '24px','fontWeight': 'bold'}}>
                              Scale1: {weight1}
                          </label>
                        </div>
                        <div className="form-check" style={{float: 'left', paddingLeft:'4.2rem'}}>
                          <input className="form-check-input" type="radio" name="weight_machine" id="weight_machine2"
                          checked={weightMachineType=='weight2'}  onChange={()=>{handleWeightMachineChange('weight2')}}
                          />
